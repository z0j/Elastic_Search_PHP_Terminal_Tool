...
abstract class AbstractEndpoint
{
    ...
    public function curlStr()
    {
        $uri = $this->getURI();
        $str = "curl -X GET -s '{hosts}{$uri}?pretty'";
        $body = $this->getBody();
        if (!empty($body)) {
            $str .= " -H 'Content-Type: application/json' -d";
            $str .= "'".PHP_EOL.json_encode($body, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE).PHP_EOL."'";
        }
        return $str;
    }
}
